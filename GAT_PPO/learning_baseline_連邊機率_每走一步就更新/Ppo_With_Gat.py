        ...
        with torch.no_grad():
            ...
            for i in range(self.env.num_agents):
                for j in range(self.env.num_nodes):
                    if self.env.node_rewards[j][0] == 0 and j != self.env.agent_positions[i]:
            ...
            for i in range(self.env.num_agents):
                ...
                if np.any(np.isnan(probs)) or np.sum(probs) <= 0:
                    valid_nodes = [j for j in range(self.env.num_nodes)
                                   if self.env.node_rewards[j][0] > 0 or j == self.env.agent_positions[i]]
                    if not valid_nodes:
                        valid_nodes = list(range(self.env.num_nodes))
                    probs = np.zeros(self.env.num_nodes)
                    probs[valid_nodes] = 1 / len(valid_nodes)

                a = np.random.choice(self.env.num_nodes, p=probs)
                actions.append(a)
                log_probs.append(torch.log(action_probs[i][a] + 1e-8))
    ...
    def train(self, num_episodes=50):
        ...
        for episode in range(num_episodes):
            ...
            while not done:
                ...
                _, next_value = self.model(next_graph_data)

                advantage = reward + self.gamma * next_value.detach().squeeze() - value.detach().squeeze()
                advantage = advantage.detach()

                new_action_probs, _ = self.model(graph_data)
                new_log_prob = torch.stack([
                    torch.log(new_action_probs[i][actions[i]] + 1e-8)
                    for i in range(self.env.num_agents)
                ]).mean()
                ...
                critic_loss = (reward + self.gamma * next_value.detach().squeeze() - value.squeeze()) ** 2
            ...
            logs.append([episode, total_reward, np.mean(step_losses)])
            if episode % 10 == 0:
                print(f"[Episode {episode}] Total Reward: {total_reward:.2f} | Avg Loss: {np.mean(step_losses):.4f}")

        print(f"訓練時間：{time.time() - start_time:.2f} 秒")
        return pd.DataFrame(logs, columns=["Episode", "Total Reward", "Avg Loss"])
