...
def build_er_graph(n_nodes=2643, n_edges=6000):
    ...
    for attempt in range(max_attempts):
        ...
        print(f"無法在 {max_attempts} 次內產生連通圖 (n={n_nodes}, m={n_edges}) \n")
        safe_m = int(0.55 * n_nodes * math.log(n_nodes))
        for _ in range(max_attempts):
            G = nx.gnm_random_graph(n=n_nodes, m=safe_m)
            if nx.is_connected(G):
                break
        else:
            raise ValueError(f"無法在 {max_attempts} 次內產生替代連通圖 (n={n_nodes}, m={safe_m})")
        print(f"產生 n ={n_nodes}, m={safe_m} 的替代連通圖")
