...
def load_input_graph(file_path="../../dataset/road-euroroad.xlsx"): #輸入讀取檔案路徑
...
def evaluate(num_agents, total_budget, per_agent_budget, dataset, model_path="ddtm_memsafe_final.pt"):
    ...
    for episode in range(10):
        ...
        env = MATPEnv(G, rewards, num_agents=num_agents, total_budget=total_budget, per_agent_budget=per_agent_budget)
        ...
        while not done and step_count < max_step:
            ...
            with torch.no_grad():
                ...
                actions, _, _ = model.select_action(logits, temperature=1.0)
